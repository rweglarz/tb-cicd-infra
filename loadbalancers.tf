...
resource "aws_lb_target_group_attachment" "fw1" {
  ...
  target_id        = "${aws_network_interface.FW1-UNTRUST.private_ip}"
  port             = 80
}

resource "aws_lb_target_group_attachment" "fw2" {
  target_group_arn = "${aws_lb_target_group.fw-tg.arn}"
  target_id        = "${aws_network_interface.FW2-UNTRUST.private_ip}"
  port             = 80
}
...
resource "aws_lb_target_group_attachment" "web1" {
  ...
  target_id        = "${aws_network_interface.web1-int.private_ip}"
  port             = 80
}

resource "aws_lb_target_group_attachment" "web2" {
  target_group_arn = "${aws_lb_target_group.web-tg.arn}"
  target_id        = "${aws_network_interface.web2-int.private_ip}"
  port             = 80
}
